...
const intern = async (props) => {
  ...
  const { header, section1, section2, section3, serverTypes } =
    fsData.functions.server.intern;

  for (const serverType of serverTypes) {
    // get maplist data
    const fsMaplist = readMaplist(
      serverType.mapListFile,
      "intern server @ fsMaplist"
    );

    // get pug server
    const pugServer = [];
    fsServers
      .filter((s) => serverType.server.includes(s.name))
      .forEach((s) => pugServer.push(...s.servers));

    const maxTitleLength = pugServer
      .map((c) => c.title.length)
      .reduce((prev, curr) => (curr > prev ? curr : prev), 10);

    const channelId = pathReducer(serverType.channel, fsData.channel);
...
${header.map((d) => `[tr][td][center][size=10]${d}[/td][/tr]`).join("")}
...
[/table]`;
    ...
    channelDescription += "[/table]";
    channelDescription += "[table]";
    for (const ruleLine of section1) {
